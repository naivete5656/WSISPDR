...
% compute the per object statistics
stats = regionprops(BW, 'Area','Perimeter','PixelIdxList');
% loop over the objects
for i = 1:numel(stats)
  % extract the perimeter and area
  p = stats(i).Perimeter;
  a = stats(i).Area;
  % compute the objects circularity
  circ = ((4*pi)*a)/(p^2);
  % if that circularity is below the threshold, remove the object
