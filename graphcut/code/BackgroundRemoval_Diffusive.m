...
%     imgInit = imgInit - 3*tmp2;
    ...
    tmp3 = imfilter(double(im),fspecial('gaussian',31,5),'replicate');
    tmp3 = (tmp3-min(tmp3(:)))/(max(tmp3(:))-min(tmp3(:)));
    tmp = im - tmp3.*mean(im(:));
    tmp3 = imfilter(double(im),fspecial('gaussian',101,17),'replicate').*maskC;
    tmp = im - 2*tmp3;
    tmp2 = imfilter(double(im),fspecial('gaussian',301,50),'replicate');
